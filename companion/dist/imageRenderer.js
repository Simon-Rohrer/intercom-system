...
export function renderButtonImage(state, options = {}) {
...
function renderWithCanvas(state, opts) {
    ...
    if (primary) {
        if (secondary) {
            ...
            const primaryStartY = primaryCenterY -
            ...
            ctx.fillText(secondaryLines[0] || secondary, canvas.width / 2, Math.round(canvas.height * (hasActionHeader ? 0.77 : 0.68)));
        }
        else {
...
function streamDeckActionHeaderLabel(actionType) {
...
function fitText(ctx, text, maxWidth, initialSize, weight, minSize = 12) {
...
function generateSolidColorPNG(width, height, _hexColor) {
