    ...
    const actions = {
        trigger_synced_button: {
            ...
            callback: async (event) => {
                ...
                const phase = String(event.options.phase || "down") === "up" ? "up" : "down";
                const sourcePageNumber = Number(event.options.sourcePageNumber ?? -1);
                const targetRoleId = String(event.options.roleId || "").trim();
                const hasSourcePage = Number.isInteger(sourcePageNumber) && sourcePageNumber >= 0;
                const button = hasSourcePage
                    ? targetRoleId
                        ? self.getPresetProfileButtonConfig(targetRoleId, sourcePageNumber, buttonIndex)
                ...
                self.log("info", `Trigger slot ${buttonIndex} page=${hasSourcePage ? sourcePageNumber : self.currentPageNumber} phase=${phase} action=${actionType} label=${label}`);
                if (hasSourcePage) {
                ...
                if (actionType === "page_up" || actionType === "page_down") {
                    if (phase === "down") {
                        ...
                        self.dispatchBridgeCommand({ command: actionType }, `trigger_synced_button slot=${buttonIndex} phase=${phase} direct=${actionType}`);
                    }
                    return;
                }
                if (actionType === "page_back") {
                    if (phase === "down") {
                        ...
                        self.dispatchBridgeCommand({ command: "page_back" }, `trigger_synced_button slot=${buttonIndex} phase=${phase} direct=${actionType}`);
                    }
                    return;
                }
                if (actionType === "page_jump" || actionType === "page_home") {
                    if (phase === "down") {
                        const targetPage = actionType === "page_home" ? 0 : (button?.action?.targetPage ?? 0);
                        self.applyLocalPageJump(targetPage);
                        self.dispatchBridgeCommand(actionType === "page_home"
                            ? { command: "page_home" }
                            : { command: "page_jump", pageNumber: targetPage }, `trigger_synced_button slot=${buttonIndex} phase=${phase} direct=${actionType} target=${targetPage}`);
                    }
                    return;
                }
                self.dispatchBridgeCommand({
