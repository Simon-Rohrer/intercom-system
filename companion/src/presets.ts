...
function presetIdPart(value: string | number): string {
  return String(value)
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "_")
...
function rolePresetCategory(self: ModuleInstance, profile?: CompanionPresetProfile): string {
  const roleLabel = cleanLabel(
    profile?.roleName ||
      profile?.roleId ||
  ...
  const profileLabel = cleanLabel(
    profile?.username || self.discovery.username || self.config.username || "",
    "Default profile",
  );
  return `Kesher / ${roleLabel} / ${profileLabel}`;
}

function pageLabel(page: StreamDeckPageConfig): string {
...
function buildProfileButtonPreset(
  ...
  const feedbacks: CompanionPresetFeedback[] = isIndicator
...
function sortedProfilePages(settings: StreamDeckSettings | null): StreamDeckPageConfig[] {
...
function fallbackCurrentProfile(self: ModuleInstance): CompanionPresetProfile[] {
  if (!self.profileStreamDeckSettings) return [];
  return [
    {
...
function sortedPresetProfiles(self: ModuleInstance): CompanionPresetProfile[] {
  ...
  return profiles
    .slice()
    .sort((a, b) => {
      const roleCmp = cleanLabel(a.roleName || a.roleId, "role").localeCompare(
        cleanLabel(b.roleName || b.roleId, "role"),
        undefined,
        { sensitivity: "base" },
      );
      if (roleCmp !== 0) return roleCmp;
      return cleanLabel(a.username, "profile").localeCompare(
        cleanLabel(b.username, "profile"),
        undefined,
        { sensitivity: "base" },
      );
    });
...
export function BuildPresetSignature(self: ModuleInstance): string {
